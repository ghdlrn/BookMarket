	...
	private void menuIntroduction() {
		...
		bt1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				mPagePanel.removeAll();// 패널(mPagePanel)에 표시된 구성요소 모두 삭제
				mPagePanel.add("고객 정보 확인", new GuestInfoPage(mPagePanel));
				// 패널(mPagePanel)에 GuestInfoPage의 내용 출력

				mPagePanel.revalidate();// 구성 요소의 가로 세로 속성을 변경하여 호출
		...
		bt3.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (mCart.mCartCount == 0)
					JOptionPane.showMessageDialog(bt3, "장바구니에 항목이 없습니다", "장바구니 상품 비우기", JOptionPane.ERROR_MESSAGE);
				else {
					mPagePanel.removeAll();
					menuCartClear(bt3);
			...
			public void actionPerformed(ActionEvent e) {
				if (mCart.mCartCount == 0)
					...
					if (mCart.mCartCount == 0)
						...
						mCart.removeCart(cartList.mSelectRow); // 장바구니에서 선택 항목 삭제하기
						cartList.mSelectRow = -1;
					}
				}
				mPagePanel.add("장바구니 항목 삭제하기", new CartItemListPage(mPagePanel, mCart));
				mPagePanel.revalidate();
				mPagePanel.repaint();
			}
		});
